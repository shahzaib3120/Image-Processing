...
# ╔═╡ bdda6ecf-1f4a-4df5-8dc4-aaff8522b319
begin 
	using Images
	img = load("yoda.jpg")
end

# ╔═╡ c8a6e249-475a-49b4-8c53-41da1063fc81
begin
	using PlutoUI
	@bind intensity Slider(0:10, show_value=true)
end


# ╔═╡ b76e26fe-2ccc-11ec-12be-1955c09b78e9
begin
	url = "https://i.insider.com/5e32f2a324306a19834af322?width=1136&format=jpeg"
	download(url , "yoda.jpg")
end

# ╔═╡ edad39cd-2647-4fc0-b886-383869e48b79
begin 
	function compress(image,ratio)
		return image[1:ratio:end,1:ratio:end]
	end
end

# ╔═╡ 67c75290-ec09-425a-93d3-c6ab3fa83ca2
begin 
	function green_filter(color, value)
		return RGB(color.r-value,color.g,color.b-value)
	end
end


# ╔═╡ b2fb6574-d6cd-4da3-812c-42533b206a1e
green_filter.(compress(img,2), intensity*0.1 )

# ╔═╡ 6bb78925-e640-49a1-a900-1cb943c66ac7
begin 
	function blur_2(image, value,skip) 
		# uses diagnoal blur and smudges.. we do not want that 
		# look for TRUE neighbours  for solution
		half = value÷2
		(h,w)= size(image)
		new = copy(image)
			for i in 1:h
...
# ╔═╡ 99eb3f75-a336-4cc5-845c-874acb2813e0
begin
	function blur(image, value,skip)
		half = value
		dim = value*8
		(h,w) = size(image)
		new = copy(image)
...
# ╔═╡ 58c8f34f-4e68-4467-b55a-8634f440464b
[blur(compress(img,2), intensity,0) compress(img,2)]
